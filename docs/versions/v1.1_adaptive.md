...
Enhanced All Weather Strategy with **adaptive rebalancing** (3% drift threshold). The strategy achieved **7.58% annual return** with **6.71% volatility** and **-6.55% max drawdown** over 8 years, delivering **72% commission savings** compared to v1.0 while improving returns.
...
strategy = AllWeatherV1(
    ...
    rebalance_threshold=0.03,        # 3% drift threshold (KEY!)
...
- 3% drift tolerance allows momentum to persist
